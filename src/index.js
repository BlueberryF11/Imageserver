...
async function handleUpload(request, env) {
  ...
  if (!token) return corsResponse(JSON.stringify({ error: 'UPLOAD_TOKEN is not configured on the Worker.' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  if (!authHeader || !authHeader.startsWith('Bearer ')) return corsResponse(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
  if (authHeader.slice(7) !== token) return corsResponse(JSON.stringify({ error: 'Invalid token' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
  ...
  try {
    ...
    return corsResponse(JSON.stringify({ error: 'Upload failed', details: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
...
<style>
*{box-sizing:border-box}body{margin:0;min-height:100vh;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#09090b;color:#f4f4f5;display:flex;align-items:center;justify-content:center;padding:24px}.card{width:min(680px,100%);background:#151518;border:1px solid #2d2d33;border-radius:20px;padding:28px;box-shadow:0 20px 60px #0008}h1{margin:0 0 6px;font-size:28px}.sub{color:#a1a1aa;margin-bottom:26px}.drop{border:2px dashed #4b4b55;border-radius:16px;padding:42px 20px;text-align:center;cursor:pointer;transition:.15s}.drop:hover,.drop.drag{border-color:#8b5cf6;background:#8b5cf611}.drop strong{display:block;font-size:18px;margin-bottom:8px}.drop span{color:#a1a1aa}.row{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:18px 0}label{font-size:13px;color:#a1a1aa;display:block;margin-bottom:7px}input{width:100%;padding:12px;border:1px solid #34343b;border-radius:10px;background:#0d0d0f;color:#fff;font:inherit}button{width:100%;padding:13px;border:0;border-radius:10px;background:#8b5cf6;color:#fff;font-weight:700;font-size:15px;cursor:pointer}button:disabled{opacity:.5;cursor:not-allowed}.files{margin:16px 0;color:#d4d4d8;white-space:pre-line}.status{margin-top:16px;padding:12px;border-radius:10px;background:#0d0d0f;color:#a1a1aa;white-space:pre-wrap}.hint{margin-top:18px;color:#71717a;font-size:12px}.hidden{display:none}@media(max-width:600px){.row{grid-template-columns:1fr}}
</style></head><body><div class="card">
<h1>🍓 Blueberry Fruitsy Image Server</h1><div class="sub">Upload artwork directly to your gallery's image server.</div>
<div class="row"><div><label>Collection</label><input id="collection" value="artwork" placeholder="e.g. artwork"></div><div><label>Upload token</label><input id="token" type="password" placeholder="Your UPLOAD_TOKEN"></div></div>
<div id="drop" class="drop"><strong>Drop artwork here</strong><span>or click to choose one or more image files</span><input id="files" class="hidden" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/bmp,image/svg+xml" multiple></div>
<div id="fileList" class="files">No files selected.</div><button id="upload" disabled>Upload artwork</button><div id="status" class="status">Ready.</div>
<div class="hint">Your token is only sent to this server over HTTPS. Do not put it in your GitHub repository.</div></div>
<script>
...
let files=[];
function setFiles(list){files=[...list];fileList.textContent=files.length ? files.map(function(f){return f.name+' ('+Math.round(f.size/1024)+' KB)';}).join('\\n') : 'No files selected.';upload.disabled=!files.length;}
drop.onclick=function(){filesInput.click();};filesInput.onchange=function(){setFiles(filesInput.files);};drop.ondragover=function(e){e.preventDefault();drop.classList.add('drag');};drop.ondragleave=function(){drop.classList.remove('drag');};drop.ondrop=function(e){e.preventDefault();drop.classList.remove('drag');setFiles(e.dataTransfer.files);};
upload.onclick=async function(){const token=document.getElementById('token').value.trim(),collection=document.getElementById('collection').value.trim()||'artwork';if(!token){status.textContent='Enter your upload token first.';return;}upload.disabled=true;let ok=0;for(const file of files){status.textContent='Uploading '+file.name+'...';const form=new FormData();form.append('file',file);form.append('collection',collection);try{const r=await fetch('/upload',{method:'POST',headers:{Authorization:'Bearer '+token},body:form});const data=await r.json();if(!r.ok)throw new Error(data.error||'Upload failed');ok++;}catch(e){status.textContent+='\\n❌ '+file.name+': '+e.message;}}status.textContent='Done. '+ok+'/'+files.length+' file(s) uploaded.'+(ok<files.length?' Check the token and file names.':'');upload.disabled=false;};
</script></body></html>`;

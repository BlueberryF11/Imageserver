 ...
 * collections.json and sitemap.xml are generated from objects under collections/<collection>/<file>.
 */
...
export default {
  ...
  async scheduled(event, env) {
    await regenerateMetadata(env, env.PUBLIC_BASE_URL || '');
  },
};
...
async function handleUpload(request, env) {
  ...
  try {
    ...
    if (!collection) return json({ error: 'Invalid collection name.' }, 400);
    ...
    await regenerateMetadata(env, new URL(request.url).origin);
    ...
    return json({
      ...
      file: {
        ...
        collection,
        key,
        previewUrl: `/collections/${encodeURIComponent(collection)}/${encodeURIComponent(filename)}`,
        downloadUrl: `/collections/${encodeURIComponent(collection)}/${encodeURIComponent(filename)}?download=1`,
      },
      sitemap: '/sitemap.xml',
    }, 201);
  } catch (error) {
...
async function handleGetImage(path, env) {
  try {
    ...
    const requestUrl = new URL(path, 'https://image.local');
...
async function handleSitemap(request, env) {
  ...
  const xml = buildSitemapXml(collections, base);

  await env.METADATA.put('sitemap.xml', xml, {
    httpMetadata: { contentType: 'application/xml; charset=utf-8' },
    metadata: { generated: new Date().toISOString(), imageCount: String(countImages(collections)) },
  });
...
async function regenerateMetadata(env, baseUrl) {
  const collections = await generateCollectionsJson(env);
  await saveCollections(env, collections);

  if (baseUrl) {
    const xml = buildSitemapXml(collections, baseUrl);
    await env.METADATA.put('sitemap.xml', xml, {
      httpMetadata: { contentType: 'application/xml; charset=utf-8' },
      metadata: { generated: new Date().toISOString(), imageCount: String(countImages(collections)) },
    });
...
function buildSitemapXml(collections, baseUrl) {
  const base = String(baseUrl || '').replace(/\/$/, '');
  const urls = [];

  for (const collection of collections) {
    for (const image of collection.images) {
      const imageUrl = `${base}${image.url}`;
      urls.push(`  <url>\n    <loc>${escapeXml(imageUrl)}</loc>\n    <lastmod>${image.uploaded ? new Date(image.uploaded).toISOString() : new Date().toISOString()}</lastmod>\n  </url>`);
    }
  }
...
function countImages(collections) {
  return collections.reduce((total, collection) => total + collection.images.length, 0);
}
...
async function handleIndex(env) {
  const collections = await generateCollectionsJson(env);
  const total = countImages(collections);
      ...
      <section class="collection">
        ...
        <div class="grid">
          ${collection.images.map(image => `
            <article class="image">
              <a href="${image.url}" target="_blank" rel="noopener"><img src="${image.url}" alt="${escapeHtml(image.title)}" loading="lazy"></a>
              <div class="image-info"><span>${escapeHtml(image.title)}</span><div class="links"><a href="${image.url}" target="_blank" rel="noopener">Preview</a><a href="${image.url}?download=1">Download</a></div></div>
            </article>`).join('')}
        </div>
      </section>`).join('')
